...
int main(int argc, char *argv[])
{
	...
	if (argc == 1)
	{
		...
		test_groupRangeData tGroupRangeData;
		QTest::qExec(&tGroupRangeData);

		test_calcExpession tCalcExpression;
		QTest::qExec(&tCalcExpression);

		test_compareExpression tCompareExpression;
		QTest::qExec(&tCompareExpression);
		return a.exec();
	}
	else
	{
