...
/*!
* Вернуть значение exprNodeType от текста QString
* \param[in] nameOfType - текст содержать тип узла 
* \return - значение типа exprNodeType
*/
exprNodeType returnExprNodeType(QString nameOfType)
{
	if (nameOfType == "const") return const1;  // Сложение
	if (nameOfType == "var") return var1;     //Переменная  
	if (nameOfType == "plus") return plus1;   //Константа
	if (nameOfType == "minus") return minus;  // Вычитание
	if (nameOfType == "multi") return multi;  // Умножение
	if (nameOfType == "divide") return divide;  // Деление
	if (nameOfType == "mod") return mod;            //Остаток от деления
	if (nameOfType == "prefixInc") return prefixInc;  // Префиксный инкремент
	if (nameOfType == "prefixDec") return prefixDec;  // Префиксный декремент 
	if (nameOfType == "postfixInc") return postfixInc;  // Постфиксный инкремент
	if (nameOfType == "postfixDec") return postfixDec;  //  Постфиксный декремент
	if (nameOfType == "assignment") return assignment;  //  Присваивание
	if (nameOfType == "addAssign") return addAssign;   // Присваивание со сложением
	if (nameOfType == "subAssign") return subAssign;  // Присваивание с вычитанием
	if (nameOfType == "multiAssign") return multiAssign;   //Присваивание с умножением
	if (nameOfType == "divideAssign") return divideAssign;   //Присваивание с делением
	if (nameOfType == "abs") return abs1;   // Модуль вычисления 
	if (nameOfType == "pow") return pow1;   // Возведение в степень 
	if (nameOfType == "sqrt") return sqrt1;  // Извлечение квадратного корня 
	if (nameOfType == "fmax") return fmax1;  // Наибольшее значение среди двух значений
	if (nameOfType == "fmin") return fmin1;  // Наименьшее значение среди двух значений

	exprNodeType tempNode;
	...
	while (!xmlStream.atEnd())
	{
		...
		if (xmlStream.name().toString() == "formula1" || xmlStream.name().toString() == "formula2")
		{
			...
			if (xmlStream.isStartElement())
			{
				...
				currentNode->type = returnExprNodeType(xmlStream.name().toString());
...

		{
			...
			if (xmlStream.isStartElement())
			{
				...
				newNode->type = returnExprNodeType(xmlStream.name().toString());
		...
		else
		{
			...
			else if (xmlStream.name().toString() == "var")
			{
				...
				newNode->type = returnExprNodeType(xmlStream.name().toString()); //Установить поле type у newNode в значение типа данных.
